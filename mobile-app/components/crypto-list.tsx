    ...
    <TouchableOpacity style={styles.listItem}>
      <View style={{display:'flex', flexDirection:'row', justifyContent:'space-between', width:'100%'}}>
        <View style={{flexDirection:'row', alignItems:'center',justifyContent:'flex-start', width:180}}>
          ...
          <Text style={[styles.itemPrice,styles.w180, styles.alignLeft]}>
            {(item.price && typeof item.price === 'number')?`${item.price.toFixed(2)}`:''}
          </Text>
      </View>
    </TouchableOpacity>
  );
...
export default function CryptoList ({ category }:{category: MarketCategory}){
  ...
  <View style={{ flex: 1, flexDirection:'column' }}>
    <View style={{ flexDirection:'row', display:'flex',justifyContent:'space-between'}}>
      ...
      <Text style={[styles.header,styles.w180, styles.alignLeft]}>价格</Text>
    </View>
    <FlashList data={data}
